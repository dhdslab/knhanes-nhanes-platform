...
if(isTRUE(cfg$skip_table1)){
  t1<-data.frame(Characteristic=character()); write.csv(t1,"engine_table1.csv",row.names=FALSE)
} else {
...
# ── Regression switches on outcome type (binary = logistic OR, continuous = linear beta) ──
